...
pre-change behaviour is equally worth capturing once — proposing and
passing a World's Fair on the current build should produce
`resolution_failed`, which is the direct evidence this document is
reasoning about from source alone.
